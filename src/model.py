...
class SepConv(nn.Module):
    def __init__(self, in_channels: int , out_channels:int, kernel_size:int, stride=1, padding=0,depth_count:int=2, is_last=False ,growth_factor=1,groups:int=1) -> None:
        ...
        last_layer=[nn.Conv2d(in_channels=growth,out_channels=out_channels,kernel_size=1,groups=groups, bias=False),
                           nn.BatchNorm2d(out_channels),
                           activation] if is_last else [nn.Conv2d(in_channels=growth,out_channels=out_channels,kernel_size=1,groups=1, bias=True)]
...
class MyModel1(nn.Module): #new
    def __init__(self, num_classes: int = 1000) -> None:
        ...
        self.Dconvs=nn.Sequential(
            ...
            SepConv(80,num_classes,3,padding=1, growth_factor=num_classes//80,depth_count=3, is_last=True),
